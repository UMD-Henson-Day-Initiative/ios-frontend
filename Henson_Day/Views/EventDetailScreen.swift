...
struct CollectionScreen: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                Color.hensBackground.ignoresSafeArea()
...
private struct BuntingView: View {
    ...
    private let flagColors: [Color] = [
        .hensRed, .hensYellow, .hensRedSoft, .hensYellowSoft,
        .hensRed, .hensYellow, .hensRedSoft, .hensYellowSoft
    ]
    ...
    var body: some View {
        Canvas { context, size in
            ...
            context.stroke(rope, with: .color(Color.hensYellow.opacity(0.9)), lineWidth: 1.5)
            ...
            for i in 0..<flagCount {
                ...
                context.stroke(flag, with: .color(Color.hensYellow.opacity(0.6)), lineWidth: 1)
            }
        }
        .frame(height: 60)
    }
    ...
    var body: some View {
        VStack(spacing: 6) {
            Text(title.uppercased())
                ...
                .tracking(3)
                .foregroundStyle(Color.hensYellow)

            // Small dot row beneath title
            HStack(spacing: 5) {
                ForEach(0..<7, id: \.self) { _ in
                    Circle()
                        .fill(Color.hensYellowSoft.opacity(0.80))
        ...
        .background(Color.hensRed.opacity(0.85))
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .strokeBorder(Color.hensYellow, lineWidth: 2)
        )
        .cornerRadius(12)
    }
}
    ...
    var body: some View {
        VStack(spacing: 2) {
            Text(label.uppercased())
                ...
                .tracking(1)
                .foregroundStyle(Color.hensDimText)

            Text(value)
                .font(.system(size: 18, weight: .bold, design: .rounded))
                .foregroundStyle(Color.hensRed)
        }
        .frame(maxWidth: 100)
        .padding(.vertical, 8)
        .background(Color.hensCream)
        .overlay(
            RoundedRectangle(cornerRadius: 10)
                .strokeBorder(Color.hensYellow, lineWidth: 1.5)
        )
        .cornerRadius(10)
    }
}
...
private struct BadgesSectionView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            .padding(.vertical, 10)
            .background(Color.hensYellowSoft)
            .overlay(
                RoundedRectangle(cornerRadius: 10)
                    .strokeBorder(Color.hensYellow, lineWidth: 1.5)
            )
    ...
    var body: some View {
        ZStack {
            VStack(spacing: 5) {
                ...
                ZStack {
                    Circle()
                        .fill(isUnlocked ? Color.hensYellowSoft : Color.hensMid.opacity(0.5))
                        .frame(width: 48, height: 48)
                    Circle()
                        .strokeBorder(
                            isUnlocked ? Color.hensYellow : Color.hensFadedText.opacity(0.4),
                            lineWidth: 2
                        )
                        .frame(width: 48, height: 48)
                    Image(systemName: isUnlocked ? "star.fill" : "star")
                        .foregroundStyle(isUnlocked ? Color.hensRed : Color.hensFadedText.opacity(0.5))
                        .font(.system(size: 20))
                }
                ...
                Text("+\(collectible.points) pts")
                    .font(.system(size: 9, weight: .bold, design: .rounded))
                    .foregroundStyle(isUnlocked ? Color.hensDimText : Color.hensFadedText.opacity(0.4))
            }
            .padding(8)
            .frame(maxWidth: .infinity, minHeight: 100)
            .background(isUnlocked ? Color.hensWarm : Color.hensMid.opacity(0.35))
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(
                        isUnlocked ? Color.hensRed : Color.hensDimText.opacity(0.5),
            ...
            if !isUnlocked {
                ...
                Image(systemName: "lock.fill")
                    .foregroundStyle(Color.hensFadedText.opacity(0.7))
                    .font(.system(size: 16))
            }
        }
    }
...
private struct WhimsicalItemRowView: View {
    ...
    var body: some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .strokeBorder(Color.hensYellow, lineWidth: 1.2)
        )
        .cornerRadius(12)
    }
}
...
private struct CollectibleDetailCardScreen: View {
    ...
    var body: some View {
        ...
        .toolbarBackground(Color.hensRed, for: .navigationBar)
